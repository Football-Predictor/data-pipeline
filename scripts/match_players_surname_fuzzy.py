#!/usr/bin/env python3
"""Targeted surname-based fuzzy pass for unmatched players.

- For unmatched SB players, extract surname (last token)
- If surname token maps to <= MAX_SURN_CANDIDATES FIFA names, run fuzzy among those
- Accept if score >= 75, else mark review if >=70
"""
# /// script
...
def run_surname_pass():
    review = pd.read_csv(REVIEW_P, dtype={'player_id_sb': int})
    # ensure writable dtypes for candidate columns
    ...
    if 'candidate_name' not in review.columns:
        ...
        review['candidate_name'] = review['candidate_name'].astype(object)
    ...
    for idx, r in unmatched.iterrows():
        sname = surname_of(r['player_name_sb'])
        if not sname:
            continue
        cand_idxs = token_index.get(sname, [])
        if not cand_idxs or len(cand_idxs) > MAX_SURN_CANDIDATES:
            continue
        choices = [norm_list[i] for i in cand_idxs]
        ...
        global_idx = cand_idxs[local_idx] if isinstance(local_idx, int) else None
        ...
        if sscore >= ACCEPT_SCORE:
            review.at[idx,'status']='accepted_surname'
            review.at[idx,'candidate_fifa_id']=fid
            review.at[idx,'candidate_name']=lrow.get('short_name')
            review.at[idx,'score']=int(sscore)
            promoted.append({'player_id_sb':int(r['player_id_sb']),'player_name_sb':r['player_name_sb'],'fifa_id':fid,'score':int(sscore),'method':'surname_fuzzy'})
            updated+=1
        elif sscore >= REVIEW_SCORE and sbg==fbg and sbg!='UNK':
            review.at[idx,'status']='accepted_surname_pos'
            review.at[idx,'candidate_fifa_id']=fid
            review.at[idx,'candidate_name']=lrow.get('short_name')
            review.at[idx,'score']=int(sscore)
            promoted.append({'player_id_sb':int(r['player_id_sb']),'player_name_sb':r['player_name_sb'],'fifa_id':fid,'score':int(sscore),'method':'surname_pos'})
            updated+=1
